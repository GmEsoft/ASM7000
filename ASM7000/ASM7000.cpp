...
namespace gmesoft
{
	const char title[] = "** TMS-7000 Tiny Assembler - v0.3.1-alpha+dev - (C) 2024 GmEsoft, All rights reserved. **";
	...
	void asm7000( int argc, const char* argp[] )
	{
		...
		for ( int i=0; i<0x100; ++i )
		{
			...
			sstr << "P" << i;
			string label = sstr.str();
			Arg arg = { ARG_PORT, word( i + 0x100 ), label, "" };
			symbols.addSymbol( label, arg );
		}
		...
		for ( pass=1; pass<=2; ++pass )
		{
			...
			while( true )
			{
					...
					)
				{
					if ( isline )
					{
						...
						if ( !options.nocerr && !lstfile.empty() && cout.rdbuf() != cerr.rdbuf()
							&& log.isWarning() )
						{
							CDBG << sstr.str();
						}
			...
			if ( pass == 2 )
			{
				...
				if ( !lstfile.empty() && cout.rdbuf() != cerr.rdbuf() )
				{
					cerr << sstr.str();
				}
			}
